...
public class Array02 {
	...
	public static void main(String[] args) {
		...
		int [] a1 = null;
		//null:아무것도 없다는 뜻의 값
		String s1 = null; // Heap에 공간확보가 되지 않은 상태
		String s2 = ""; // Heap에 공간확보가 되고 그 주소가 저장된 상태
		
		int [] b1; // null값도 넣지 않은 상태 / 초기화되지 않은 상태
		System.out.println(a1);
		//System.out.println(b1); // 에러. 초기화되지 않은 변수의 사용은 에러
		//변수에 null을 초기화 한 것과 아무것도 초기화하지 않은 것은 다릅니다.
		// null값이 들어가는 변수는 죄다 참조변수라고 생각하면됨
		
		a1 = new int[4];
		
		//2. 배열 변수의 선언과 배열의 생성을 동시에 진행
		int[]a2//아 정수 몇개를 저장할 수 있는 배열~. 
		= new int[4];
		// new에 의해서 Heap 메모리에 생성되는 모든 영역은 정수형 자료라면 0
		// 실수형 자료라면 0.0, String 자료라면 null값으로 초기화됩니다.
		System.out.println(a1[0]+" "+a1[1]+" "+a1[2]+" "+a1[3]);
		System.out.println(a2[0]+" "+a2[1]+" "+a2[2]+" "+a2[3]);
		
		double  [] d = new double[4]; // 모두 0.0으로 초기화
		String [] s = new String[4]; // 모두 null로 초기화 // 복잡
