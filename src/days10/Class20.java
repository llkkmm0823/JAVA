...
class ThisB{
	...
	private int num2;
	private int num3;
	public ThisB(int i) { // 생성자 1
		num1 = i;
		System.out.println("ThisB클래스의 생성자 1이 호출되었습니다");
	}
	
	ThisB(int n1, int n2){
		// num1 = n1; num1=i와 코드가 겹치기 때문에
		// ThisB(n1);
		this(n1); //매개변수 한 개를 갖고 있는 자신의 형제 생성자(생성자 1), 오버로딩된 생성자를 호출
		num2=n2;
		System.out.println("ThisB클래스의 생성자 2가 호출되었습니다");
	}
	
	ThisB(int n1, int n2, int n3){
		this(n1, n2);  //매개변수 두 개를 갖고 있는 오버로딩된 생성자(생성자 2)를 호출
		num3 = n3;		System.out.println("ThisB클래스의 생성자 3이 호출되었습니다");
