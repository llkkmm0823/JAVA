...
//static 변수와 같이 프로그램 구동 전에 메모리에 로딩되어서 객체 생성 없이 호출될 수 이는 클래스의 멤버메서드
// static 변수 사용과 같이 매서드 이름 앞에 클래스 이름과 (.)으로 연결해서 사용합니다
// 객체 생성과 상관 X
// 대표 사용 예 : Math.random();
...
class StaticC{
	...
	// final : 해당 변수의 값이 선언과 동시에 초기화 된 값으로 고정
	// 수정할 수 없음 . 수정을 시도하면 에러발생
	// static 멤버 뿐만 아니라 final로 선언되는 모든 변수와 메서드의 변경을 허락하지 않고자 할 때 사용
	// 프로그램 내부의 변경되면 안되는 지표가 될 데이터에 대해서 이와 같이 static final 키워드를 붙여 사용
	//
}
public class Class25 {
	...
	public static void main(String[] args) {
		...
		// 에서 확인가능_상식적으로 알아두삼. 즐찾 ㄱㄱ
		
		// static 메서드는 클래스 이름을 앞에 두고 호출되고, 인스턴스 메서드는 객체 이름을 앞에 두고 호출됨
		double d = Math.random();
		System.out.println("Math.random()이 발생한 난수 : " + d);
		
		Random rd = new Random();
		int i = rd.nextInt();
		System.out.println("Random클래스의 nextInt()가 발생한 난수 : " + i);

		// java.lang 패키지는 별도의 import없이 미리 import 되어 있는 패키지임
		System.out.println("Math 클래스의 static PI 변수값 : "+ Math.PI);
		
		//abs의 사용 
		int num = -10;
		System.out.printf("num=%d, num 변수의 절댓값 = %d\n", num,Math.abs(num));
		//sqrt의 사용
		num=9;
		System.out.printf("num=%d, num 변수의 제곱근 = %.2f\n", num,Math.sqrt(num));
		
		//Integer 클래스 (int 타입의 자료형과 매칭되는 클래스)
		System.out.println("Integer의 mac value : " +Integer.MAX_VALUE);
		...
		// 출력값 : 1.7976931348623157E308 -> 1.7976~~의 10의 308승
