...
public class MultiChatServer {
	...
	// Value : 클라이언트에 연결된 이름 (key값)으로 새엉된 dos(DataoutputStream)객체,
	// 			   key값에 해당하는 클라이언트에게 메세지를 보낼 객체
//해쉬맵에 저장된 클라이언트들의 dos 객체를 하나씩 꺼내서 한사람이 보낸 메세지를 모두에게 전송해줍니다	
	MultiChatServer() {
		clients = new HashMap();
		Collections.synchronizedMap(clients);
		//멀티 쓰레드 환경에서 단일 해쉬맵에 멀티 스레드가 동시 접근하여 값을 변경하는 것을 막는 동기화 도구
	}
	
	//----------------------------------------------------------------------------------------------
	class ServerReciever extends Thread{
		// 프로그램의 특성상 ServerReciever가 사용할 자료들이 MultichatServer에 일부 존재하므로
		// ServerReciever를 MultiChatServer안에 넣어서 별도의 조치 없이 그 자료를 사용하게 함
		Socket socket;
		DataInputStream dis;
		...
		ServerReciever ( Socket s){
			...
			try {
				...
				//채팅참여자 한 명이 보내온 메세지를 실시간으로 받아서 바로 모든 참여자에게 전송
				// 실시간 수신에 Thread가 필요하지만 그 때 송신고 같이 할 예정이므로 송신을 위한 Thread는 별도로 만들지 x
