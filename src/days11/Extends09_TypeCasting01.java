...
public class Extends09_TypeCasting01 {
	...
	public static void main(String[] args) {	
		...
		short s = 10;
		int i = s; // 자동 형 변환(short -> int) : 작은 용향에서 큰 용량 변수로
		int j = 10;
		// short k = j; // 에러(int -> short) : 큰 용량에서 작은 용량의 변수로는 데이터 손실이 발생
		// 해결 방법은 강제 캐스팅 연산자를 앞에 붙여 사용
		short k = (short)j;
		// 강제캐스팅도 두 자료형 간의 호환성이 있어야 가능
		// String str = i; //에러 String.valueOf(i);
		// int p = "1234"; // 에러 Integer.parseInt("1234");
		
		// 클래스의 레퍼런스 변수 간의 형 변환은 각 레퍼런스의 접근 범위에 영향을 받음
		// 서로 다른 클래스 객체간의 형 변환은 허용되지 않음
		NormalA a1 = new NormalA();
		//NormalB b1 = a1; // 에러 - NormalA의 객체 인스턴스로 NormalB 참조변수에 저장 X
		//NormalB b1 = new NormalA(); // 에러 - NormalA의 뉴 인스턴스로 NormalB 참조변수에 저장 X
