...
public class CalendarClass01 {
	...
	public static void main(String[] args) {
		//싱글턴방식의 클래스.
		//생성자가 private으로 접근 차단됨(외부에서 접근 금지 -> 외부에서 new Calendar() 사용안됨)
		//유일하게 실행된 new Calendar()는 클래스 내부에서 실행되어 클래스 내부에 단 하나만 존재
		//그 레퍼런스 변수가 갖고 있는 주소를 리턴하는 public static형태의 getInstance메서드로 인스턴스의 주소를 리턴받아 사용
		//이후 다시 getInstance()메서드가 실행되어도 현재 Heap에 이미 있는 인스턴스의 주소가 리턴됨
		Calendar tommorow = Calendar.getInstance(); // 특별히 날짜 지정을 하지 않으면 현재 날짜는 오늘 날짜
		//공유된 인스턴스에서 현재 날짜에 관한 정보만 객체끼리 서로 다른 날짜를 따로 관리하고 그 외 내용이 공유됨
		
		//Calendar 클래스에는 static final 멤버변수가 다수 있음
		System.out.println("년도 상수 : "+Calendar.YEAR);
		System.out.println("월 상수 : "+Calendar.MONTH);
		System.out.println("일 상수 : "+Calendar.DATE);
		System.out.println("현재 연도 내 주 상수 : "+Calendar.WEEK_OF_YEAR);

		// 위의 static final이 사용되는 예
		...
		// Date 또는 Calendar로 만든 날짜 : 2022-10-17 // 제 각각의 정수로 이루어진 것
		// String 데이터에서 2022를 추출하려면 substring사용 -> 결과 String "2022"
		// Calendar에서 2022를 추출하려면 get사용 -> 결과 int

		int i = today.get(Calendar.MONTH);
		System.out.println("월 : (0~11) : "+ i + "월");
		// MONTH로 얻어낸 월의 값은 1월이 0 2월이 1... 12월이 11로 얻어짐
		// 컴퓨터는 0부터 세니까
		...
		System.out.println("금년의 몇 째주 : "+ i );
		i = today.get(Calendar.WEEK_OF_MONTH);
		System.out.println("현재 달의 몇 째주 : "+ i );
		...
		System.out.println("1000분의 1초(0~999) "+today.get(Calendar.MILLISECOND)); // 꼭 기억해 두기
