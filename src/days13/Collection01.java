...
// -크기가 고정되어있음, 데이터의 중간삽입과 중간 삭제에 비효율적
...
// LinkedList를 구형하고 있는 클래스 - LinkedList
...
// (검색을 위해서 사용, Key값은 중복을 허용하지 않음, Value의 값은 중복을 허용)
...
public class Collection01 {
	...
	public static void main(String[] args) {
		...
		Vector v = new Vector();//import 필요
		//동적 배열의 객체 생성 초기값으로 크기를 지정할 수 있지만 통상 크기 지정 없이 사용함
		ArrayList a = new ArrayList(); //import 필요
		...
		// 이 둘은 모두 10과 20과 30이 저장된 객체의 참조값(주소)이 저장됨
		// 10, 20, 30이 직접 저장되는 것 아님
		
		// add 메서드의 매개변수가 Object 형
		// ㄴ어떤 자료형의 객체이든 저장이 가능
		// 10, 20, 30은 객체가 아니지만 저장될 때 Integer 형으로 자동변환됨
		
		int b = (Integer)a.get(0); 
		// Object형 객체라 int에 안들어가서 에러가 뜸
		//Integer로 강제 캐스팅해줘야함
		
		//Vector, ArrayList 에 담겨있는 자료들의 갯수를 리턴한 메서드: size()
		...
		System.out.printf("a[%d] = %d\t\t", i,a.get(i)); //최초 상태
		
		// 인덱스 매개변수를 추가하여 데이터 삽입-수정 가능
		a.set(2, 100); // set은 기존값을 지우고 대입
		System.out.println();
		for(int i =0; i<a.size();i++)
			//v.get(i)를 %d에 적용할 때는 강제 캐스팅이 필요없음
			System.out.printf("a[%d] = %d\t\t", i,a.get(i)); // 100을 set하고 출력
		
		a.add(2,300); // add는 기존값들을 뒤로 밀고 중간에 삽입
		System.out.println();
		for(int i=0;i<a.size();i++)
			System.out.printf("a[%d]=%d\t\t", i,a.get(i)); //300을 add하고 출력
